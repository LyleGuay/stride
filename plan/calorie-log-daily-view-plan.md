...
- [ ] **A.1 — Create calorie log migration file**
  Create `db/2-2026-02-13-calorie-log.sql`. Follow the existing migration pattern (transaction, temp table guard, schema_versions insert). This migration should:
  ...
  - No user seed data in the migration. The CLI tool (A.2) handles user creation and settings.

- [ ] **A.2 — Create user CLI tool**
...
- [ ] **A.3 — Run the migration and create dev user**
  Execute `db/2-2026-02-13-calorie-log.sql` against the DigitalOcean PostgreSQL instance (creates tables, enums, indexes). Then run the CLI tool to create a dev user (which also seeds their calorie_log_settings). Order: migration first (creates the tables), CLI second (inserts data).
